...
// ─────────────────────────────────────────────────────────────
// 墙面凸出设施的碰撞体（与 buildRoom 中的视觉模型一一对应）
// 轴对齐盒，世界坐标。家具与这些设施不可穿模。
// ─────────────────────────────────────────────────────────────
...
export function buildRoomObstacles(p: RoomParams, bumps: BumpCorners = NO_BUMPS): RoomObstacle[] {
  ...
  const W = p.width
  const H = p.height
  const obs: RoomObstacle[] = []
  const winX = p.windowEnd === 'negX' ? -L / 2 : L / 2
  const doorX = -winX
  const doorSide = p.windowEnd === 'negX' ? 1 : -1
  const leafOff = p.windowEnd === 'negX' ? 0.02 : -0.02 // 与门扇视觉模型一致
  const sillOff = p.windowEnd === 'negX' ? 0.04 : -0.04 // 与窗台板视觉模型一致

  // 踢脚线（四面墙根，高 8cm）
  for (const side of [-1, 1]) {
    obs.push({ x: 0, z: side * (W / 2 - 0.006), hx: L / 2, hz: 0.006, y0: 0, y1: 0.08 })
    obs.push({ x: side * (L / 2 - 0.006), z: 0, hx: 0.006, hz: W / 2, y0: 0, y1: 0.08 })
  }
  // 弱电箱（+Z 长墙低位）
  obs.push({ x: doorSide * (L / 2 - 1.6), z: W / 2 - 0.05, hx: 0.225, hz: 0.05, y0: 0.29, y1: 0.61 })
  // 空调内机（+Z 长墙高处）
  obs.push({ x: doorSide * (L / 2 - 1.0), z: W / 2 - 0.11, hx: 0.45, hz: 0.11, y0: H - 0.6, y1: H - 0.3 })
  // 插座
  for (const sx of [-L / 4, L / 4]) {
    obs.push({ x: sx, z: W / 2 - 0.012, hx: 0.075, hz: 0.01, y0: 0.275, y1: 0.365 })
  }
  // 门扇（贴门端墙）
  obs.push({ x: doorX + leafOff, z: p.doorOffset, hx: 0.02, hz: (p.doorWidth - 0.06) / 2, y0: 0, y1: 2.08 })
  // 门套
